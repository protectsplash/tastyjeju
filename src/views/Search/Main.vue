        ...
        <v-col class="px-12 py-16" sm="12" md="6">
          ...
          <v-row class="flex-column pa-0">
            ...
            <div class="my-6 text-body-2" :class="$style.filterDescription">
              ...
              <span class="ml-3"><strong>여행이 2일 남았습니다.</strong> 지금 바로 예약할 수 있는 숙소를 보려면 즉시 예약 필터를 사용하세요.</span>
            </div>
            <v-divider>
              inset
            </v-divider>
          ...
          <v-row class="px-0 pb-3 pt-5" :class="$style.hotelContainer" v-for="(item) in $store.state.airBnbLists" :key="item.recordid">
            <v-col class="pa-0" cols="5">
              <img v-if="item.fields.thumbnail_url" :src="item.fields.thumbnail_url" class="rounded-xl" alt style="width: 100%; height: 100%">
            ...
            <v-col class="pl-4 py-0" cols="7">
              ...
              <div class="text-body-2" :class="$style.hotelDescription">최대 인원 {{ item.fields.guests_included }}명 &middot; 침실 {{ item.fields.bedrooms }}개 &middot; 침대 {{ item.fields.beds }}개 &middot; 욕실 {{ item.fields.bathrooms }}개</div>
              <div class="text-body-2 mt-1 mb-2" :class="$style.hotelDescription">{{ splitString(item.fields.amenities, 0) }} &middot; {{ splitString(item.fields.amenities, 1) }} &middot; {{ splitString(item.fields.amenities, 2) }}</div>
              <div class="mt-16" :style="{position: 'relative', width: '100%'}">
                ...
                <div :style="{position: 'absolute', right: '-10px', top: '1px'}"><span class="font-weight-bold text-body-1">$ {{item.fields.price}} </span>/ 1박</div>
              </div>
            </v-col>
          </v-row>
          <!--    페이지네이션      -->
          <v-row class="pa-0 mt-16 flex-column" justify="center" >
            ...
            <div class="text-caption text-center mt-6" :class="$style.hotelDescription">추가 수수료가 부과됩니다. 세금도 부과될 수 있습니다.</div>
          </v-row>
        </v-col>
...
export default {
  ...
  mounted() {
    // 서치 데이터 페칭
    this.$store.dispatch('getSearchForm', this.$route.query)
      .then((res) => {
